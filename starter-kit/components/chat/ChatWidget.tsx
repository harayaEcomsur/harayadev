...
export function ChatWidget({ businessName, stacked }: { businessName: string; stacked?: boolean }) {
  ...
  return (
    <div className={`fixed right-4 z-50 ${stacked ? "bottom-24" : "bottom-4"} sm:right-6`}>
      {open && (
        <div className="mb-3 flex h-[28rem] w-[20rem] flex-col overflow-hidden rounded-2xl border border-foreground/10 bg-background shadow-2xl sm:w-[22rem]">
            ...
            {messages.map((m) => (
              <div
                ...
                className={`max-w-[85%] rounded-xl px-3 py-2 ${
                  m.role === "user" ? "ml-auto bg-primary text-white" : "bg-foreground/5"
                }`}
              >
          ...
          <form onSubmit={handleSubmit} className="flex items-center gap-2 border-t border-foreground/10 p-2">
            <input
              value={input}
              onChange={handleInputChange}
              placeholder="Escribe tu mensaje…"
              aria-label="Mensaje"
              className="flex-1 rounded-full border border-foreground/10 px-3 py-2 text-sm outline-none focus:border-primary"
            />
