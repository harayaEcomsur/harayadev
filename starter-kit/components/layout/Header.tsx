...
export function Header({ config }: { config: ClientConfig }) {
  ...
  return (
    <header className="sticky top-0 z-30 border-b border-foreground/5 bg-background/90 backdrop-blur">
      ...
      {open && (
        <nav className="border-t border-foreground/5 bg-background sm:hidden">
          <Container className="flex flex-col py-2">
            {links.map((link) => (
              <a
                ...
                className="border-b border-foreground/5 py-3.5 text-base font-medium text-foreground/80 last:border-b-0 hover:text-primary"
              >
                {link.label}
              </a>
            ))}
