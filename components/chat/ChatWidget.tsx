      ...
      {open && (
        <div className="mb-3 flex h-[28rem] w-[20rem] flex-col overflow-hidden rounded-2xl border border-line bg-card shadow-2xl sm:w-[22rem]">
          ...
          <div className="flex-1 space-y-3 overflow-y-auto px-3 py-3 text-sm">
            ...
            {error && (
              <p className="rounded-xl bg-[#10192A] px-3 py-2 text-sm text-soft">
          ...
          <form onSubmit={(e) => { if (input.trim()) trackEvent("chat_usado", { origen: "widget" }); handleSubmit(e); }} className="flex items-center gap-2 border-t border-line p-2">
            <input
              ...
              aria-label="Mensaje"
              className="flex-1 rounded-full border border-line bg-background px-3 py-2 text-sm text-foreground outline-none focus:border-primary"
            />
